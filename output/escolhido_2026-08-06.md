# 🎯 O Escolhido — NRDS (NerdWallet, Inc.)

*Communication Services · reporta 06/08 AMC · prazo de decisão: **06/08 às 21:00 de Lisboa** · gerado 2026-08-06 16:13.*

## Porque é o nº 1 de hoje
É o topo do motor entre 116 candidatos limpos do dia: retorno esperado **3.7%**, com **34%** de probabilidade calibrada de subir ≥5% e **7.7%** de subir ≥20%. Em 8 de cada 10 casos históricos análogos, o desfecho ficou entre **-14.4%** e **15.6%** — este intervalo é a régua honesta do risco.

## Os fundamentais, explicados
- **Receita a crescer 6%/ano** — crescimento moderado/baixo — o print vive mais da margem e do guidance do que da receita.
- **Margem bruta 93%** — margem alta: cada dólar extra de receita cai quase direto no lucro (alavanca operacional).
- **Altman Z'' 3.0** — balanço sólido, risco de insolvência remoto.
- **Stock-based comp 3% da receita** — baixa: os lucros são 'de verdade', pouca diluição.
- **Accruals -18%** — negativos/baixos: lucros suportados por caixa real (qualidade alta).

## O jogo das expectativas (o que decide o dia do report)
- Bateu o consenso em **51%** dos últimos 8 trimestres.
- Surpresa média recente **-28%** vs consenso — ⚠ falha o consenso POR HÁBITO — os analistas estão sistematicamente otimistas com este nome.
- MAS bateu-e-caiu em **33%** dos beats — às vezes bater não chega; as expectativas embutidas são o verdadeiro listão.
- EMI 0.67 (percentil 75 do dia) — mede cobertura de analistas + posse institucional + incentivos a 'fabricar' um beat (Johnson-Kim-So: alto EMI = +64-88bps no mês do anúncio, historicamente).

## O que o mercado está a dizer
- **Days-to-cover 6.1** — shorts levariam dias a sair: combustível de squeeze se o print for bom.
- Short interest a subir (6.7% vs quinzena anterior).
- Smirk +12.5pp (proteção OTM cara — cautela institucional).
- O mercado preça um movimento de **20.4%**; o histórico dela move **17.4%** — edge 0.85 (o bilhete está caro ou justo vs o hábito).

## O caso CONTRA (lê isto antes de decidires)
- Probabilidade de **cair**: ~46% — e quando cai, a queda média dos análogos é -10.6%.
- IC95 do EV nos 50 análogos: [-3.4%; 4.9%] — **atravessa zero**: o edge DESTE evento isolado não se distingue de nada; o sistema paga em série (~20+ eventos), nunca num tiro.
- O intervalo de 80% inclui -14.4% — perder isto num dia é cenário normal, não extremo.
- Movimento típico dela em earnings: 17.4% para QUALQUER lado; stops não protegem através de gaps.

---
*REGRA DE MEDIÇÃO (paridade backtest-live): o desfecho do sistema mede-se **fecho-a-fecho** — do fecho de HOJE ao fecho de AMANHÃ. Sair na abertura ou intradiário quebra a paridade com os números do backtest. Este é 1 evento de uma série — o veredito do sistema é ao evento ~20, nunca hoje.*
*O sistema nomeia o nº 1 pela regra pré-registada e explica porquê — a decisão e o tamanho são teus. P calibrada = frequência histórica verificada, não convicção. Brief completo (Radar, Estreantes, tabelas) em output/brief_2026-08-06.md.*